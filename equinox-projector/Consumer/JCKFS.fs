...
type ParallelConsumer private (inner : IConsumer<string, string>, task : Task<unit>, triggerStop) =
    ...
    static member Start<'M>
        (   log : ILogger, config : KafkaConsumerConfig, executingMaxDop, mapResult : (ConsumeResult<string,string> -> 'M), handle : ('M -> Async<Choice<unit,exn>>),
    ...
    /// Processor pumps until `handle` yields a `Choice2Of2` or `Stop()` is requested.
    static member Start
        (   log : ILogger, config : KafkaConsumerConfig, executingMaxDop, handle : KeyValuePair<string,string> -> Async<unit>,
            ?maxSubmissionsPerPartition, ?pumpInterval, ?statsInterval, ?logExternalStats) =
        let mapConsumeResult (x : ConsumeResult<string,string>) = KeyValuePair(x.Key, x.Value)
        ParallelConsumer.Start<KeyValuePair<string,string>>(log, config, executingMaxDop, mapConsumeResult, handle >> Async.Catch,
