...
module Log =
    ...
    module InternalMetrics =
        ...
        /// Relies on feeding of metrics from Log through to CounterSink
        /// Use Counters.CounterSink.Reset() to reset the start point (and stats) where relevant
...
type GesGateway(conn : GesConnection, batching : GesBatchingPolicy) =
    ...
    member __.TrySync log (Token.Unpack token as streamToken) (events, encodedEvents: EventData array) (isCompactionEventType) : Async<GatewaySyncResult> = async {
        ...
        match wr with
        | EsSyncResult.Conflict actualVersion -> return GatewaySyncResult.ConflictUnknown (Token.ofNonCompacting token.stream.name actualVersion)
    ...
    member __.Sync(log, streamName, streamVersion, events: Codec.IEvent<byte[]>[]) : Async<GatewaySyncResult> = async {
        ...
        match wr with
        | EsSyncResult.Conflict actualVersion -> return GatewaySyncResult.ConflictUnknown (Token.ofNonCompacting streamName actualVersion)
        | EsSyncResult.Written wr ->
            let version' = wr.NextExpectedVersion
