...
module Outcome =
    ...
    let render_ sn ham spam elapsedS =
        let share = TimeSpan.FromSeconds(match Array.length ham with 0 -> 0. | count -> elapsedS / float count)
...
type StatsBase<'outcome>(log, statsInterval, stateInterval, verboseStore, ?abendThreshold) =
    inherit Propulsion.Streams.Stats<'outcome>(log, statsInterval, stateInterval, failThreshold = TimeSpan.s 120L, ?abendThreshold = abendThreshold)
