...
public static class Todo
{
    ...
    public class State
    {
        ...
        State(int nextId, Event.ItemData[] items)
        {
            NextId = nextId;
            Items = items;
        }
    ...
    public class Service(Func<ClientId, Equinox.DeciderCore<Event, State>> resolve)
    {
        ...
        public Task<Unit> Execute(ClientId clientId, Command command) =>
            resolve(clientId).Transact(command.Interpret);
