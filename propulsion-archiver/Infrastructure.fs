...
type Equinox.CosmosStore.CosmosStoreConnector with
    ...
    member x.ConnectContext(role, databaseId, containerId, tipMaxEvents, ?tipMaxJsonLength, ?auxContainerId) = async {
        let! client = x.Connect(role, databaseId, [| yield containerId; yield! Option.toList auxContainerId |])
        return client.CreateContext(role, databaseId, containerId, tipMaxEvents, ?tipMaxJsonLength = tipMaxJsonLength) }
